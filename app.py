    ...
    try:
        ...
        if request.method == 'POST':
            ...
            for i, v in enumerate(columns.keys()):
                schema_property = columns[v]
                input_data.append(schema_property[selected[v][0]])
            pd.DataFrame([input_data], columns=columns.keys()).to_csv('prediction_files/input.csv', index=False)
        return render_template('index.html', data={'is_uploaded': is_uploaded})
